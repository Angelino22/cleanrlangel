...
from pettingzoo.butterfly import foozpong_v3
import wandb
import tyro
...
class Args:
    ...
    wandb_project_name: str = "test_project"
    wandb_entity: str = "test_entity"
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        shared_rb.add(obs, next_obs, actions, rewards, terminations, infos)
        for agent in agent_ids:
            if agent in rewards:
                episode_reward[agent_ids.index(agent)] += rewards[agent]
                adjusted_episode_reward[agent_ids.index(agent)] += rewards[agent] * (1 if agent in [offense_agent, defense_agent] else -1)
                if agent == offense_agent:
                    score_rewards_offense += rewards[agent]
                elif agent == defense_agent:
                    score_rewards_defense += rewards[agent]
                elif agent == ai1_agent:
                    score_rewards_ai1 += rewards[agent]
                elif agent == ai2_agent:
                    score_rewards_ai2 += rewards[agent]
                total_score_rewards_game += rewards[agent]
                if agent in [offense_agent, defense_agent]:
                    total_score_rewards_teamtrack += rewards[agent]
                else:
                    total_score_rewards_teamai += rewards[agent]

        game_score[offense_agent] += rewards.get(offense_agent, 0)
        game_score[defense_agent] += rewards.get(defense_agent, 0)
        game_score[ai1_agent] += rewards.get(ai1_agent, 0)
        game_score[ai2_agent] += rewards.get(ai2_agent, 0)

        if global_step % 100 == 0:
            check_for_errors()

        if sum(game_score.values()) >= 10:
